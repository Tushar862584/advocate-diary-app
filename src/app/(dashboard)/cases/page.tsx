...
export default async function CasesPage() {
  ...
  const getNextHearingDate = (caseItem: Case) => {
    if (caseItem.hearings && caseItem.hearings.length > 0 && caseItem.hearings[0].nextDate) {
  ...
  return (
    <div className="cases-page">
        ...
        <div className="overflow-hidden rounded-lg border border-slate-700 shadow-md bg-slate-800">
          <table className="min-w-full divide-y divide-slate-700">
            <thead className="bg-slate-700">
              <tr>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-300 w-[18%]">
                  Case Number
                </th>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-300 w-[42%]">
                  Title & Parties
                </th>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-300 w-[15%]">
                  Court
                </th>
                <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-300 hidden md:table-cell w-[15%]">
                  Next Hearing
                </th>
                <th scope="col" className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-slate-300 w-[10%]">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-700 bg-slate-800">
              {cases.map((caseItem, index) => (
                <tr key={caseItem.id} className="hover:bg-slate-700 transition-colors">
                  <td className="px-4 py-3 align-top">
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-blue-400 truncate">{caseItem.caseType}</span>
                      <span className="text-sm text-slate-400 truncate">#{caseItem.registrationNum}/{caseItem.registrationYear}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 align-top">
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-slate-200 line-clamp-1">{caseItem.title}</span>
                      <div className="text-xs text-slate-400 mt-1">
                        <span className="line-clamp-1">{caseItem.petitioners.map(p => p.name).join(", ")} vs {caseItem.respondents.map(r => r.name).join(", ")}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-sm text-slate-300 hidden md:table-cell align-top">
                    <div className="flex items-center">
                      ...
                      <span className="truncate">{getNextHearingDate(caseItem)}</span>
                    </div>
                  </td>
      ...
      <div className="space-y-3 sm:hidden">
        {cases.map((caseItem) => (
          <div key={caseItem.id} className="bg-slate-800 border border-slate-700 rounded-lg overflow-hidden shadow-sm">
            <div className="p-3 border-b border-slate-700">
              <div>
                <h3 className="font-medium text-blue-400 flex items-center text-sm">
                  <span className="truncate">{caseItem.caseType}</span>
                  <span className="ml-1 text-xs text-slate-400 whitespace-nowrap">#{caseItem.registrationNum}/{caseItem.registrationYear}</span>
                </h3>
                <p className="text-sm text-slate-200 mt-1.5 line-clamp-2">{caseItem.title}</p>
              </div>
              
              <div className="text-xs text-slate-400 mt-2">
                <div className="flex items-start">
                  <span className="font-medium mr-1 whitespace-nowrap">Court:</span>
                  <span className="line-clamp-1">{caseItem.courtName}</span>
                </div>
                <div className="flex items-start mt-1">
                  <span className="font-medium mr-1 whitespace-nowrap">Parties:</span>
                  <span className="line-clamp-1">{caseItem.petitioners.map(p => p.name).join(", ")} vs {caseItem.respondents.map(r => r.name).join(", ")}</span>
                </div>
              </div>
            </div>
